...
for f in *.bam; do
 ...
 8 $PROJECT_FOLDER/data/sorted $PREFIX /data/data2/scratch2/deakig/Oak/to_sort/$f
done

# get list of bam files for each assembly
BAMS=$(for f in $PROJECT_FOLDER/data/sorted/$P1*; do echo $f; done|tr  '\n' ' ')
#A=$(for f in $PROJECT_FOLDER/data/sorted/A*; do echo $f; done|tr  '\n' ' ')
# G=$(for f in $PROJECT_FOLDER/data/sorted/G*; do echo $f; done|tr  '\n' ' ')
# L=$(for f in $PROJECT_FOLDER/data/sorted/L*; do echo $f; done|tr  '\n' ' ')
# W=$(for f in $PROJECT_FOLDER/data/sorted/W*; do echo $f; done|tr  '\n' ' ')
...
# concatenate bins capturing bin name
for f in $PROJECT_FOLDER/data/taxonomy_binning/$PREFIX_BINS/*.fa; do
 sed -e "s/>k/>${f}.k/" $f >>$PREFIX.bins.fa
done

# run kaiju for taxonomic assignment of bins
...
# Total taxonomy counts - waste of time, can all be done with kaiju-addTaxonNames and a very simple R script
#f=ATTINGHAM
...
# sqlite3 has a limit on the complexity of queries - 10000 OR statements
